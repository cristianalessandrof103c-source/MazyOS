...
Também em construção: sistema web único (`sistema/`) — CRM + agente de IA no WhatsApp + financeiro, plano completo em `C:\Users\topher7\.claude\plans\um-sistema-que-crispy-pie.md`. Progresso (2026-07-07): Fase 0 (auth multi-tenant) e Fase 1 (CRM manual, board Kanban) prontas e validadas. Fase 2 (WhatsApp sandbox + agente de IA) com código completo e deployado — Edge Function `whatsapp-webhook` recebe mensagem, resolve tenant, roda o agente com as 5 tools e responde; testada ponta a ponta via simulação (só falta crédito na Anthropic pra gerar a resposta real, ver abaixo). Número de teste do WhatsApp já registrado: `120219489644135` (`whatsapp_connections` no banco).
