...
function Icon(props: SVGProps<SVGSVGElement>) {
  return <svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" {...props} />
}
...
const NAV_ICONS = {
  ...
  agente: (p: SVGProps<SVGSVGElement>) => <Icon {...p}><path d="M4 4h16v11H8l-4 4V4Z" /><circle cx="9" cy="9.5" r="0.9" fill="currentColor" /><circle cx="12" cy="9.5" r="0.9" fill="currentColor" /><circle cx="15" cy="9.5" r="0.9" fill="currentColor" /></Icon>,
...
function navItems(tenantId: string) {
  return [
    ...
    { to: `/agente/${tenantId}`, label: 'Agente', icon: NAV_ICONS.agente },
...
// Avatar padrão da plataforma (sem marca própria do tenant configurada). Logo oficial
// ainda não integrada (sistema/public/logo-bk-solutions.png) — cai pro badge "BK" via
// onError, sem quebrar a sidebar até o arquivo existir.
function DefaultBrandAvatar() {
  const [imgFailed, setImgFailed] = useState(false)
  if (imgFailed) {
    return <span className="brand-glow grid h-9 w-9 flex-shrink-0 place-items-center rounded-lg bg-violet font-display text-sm font-bold text-bg">BK</span>
  }
  return (
    <img
...
export function TenantSidebarLayout({ tenantId, children }: { tenantId: string; children: ReactNode }) {
  ...
  return (
    <div className="app-shell flex min-h-screen bg-bg text-text max-md:flex-col" style={brandingStyleFor(brandingQuery.data?.branding_json)}>
      <aside className="flex w-56 flex-shrink-0 flex-col border-r border-sidebar-border bg-sidebar max-md:w-full max-md:border-b max-md:border-r-0">
        <Link to={`/visao-geral/${tenantId}`} className="m-3 flex items-center gap-3 rounded-xl bg-sidebar-surface px-3 py-3">
          {logoUrl ? <img src={logoUrl} alt="" className="h-9 w-9 flex-shrink-0 rounded-lg object-cover" /> : <DefaultBrandAvatar />}
        ...
        <nav className="flex flex-1 flex-col gap-1.5 px-3 py-2 max-md:flex-row max-md:overflow-x-auto max-md:py-3">
        ...
        <div className="border-t border-sidebar-border p-3 max-md:flex max-md:items-center max-md:gap-1 max-md:border-t-0">
          <p className="truncate px-3 pb-2 text-xs text-sidebar-fg-faint max-md:hidden">{user?.email}</p>
